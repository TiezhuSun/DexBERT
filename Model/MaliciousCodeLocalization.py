...
class ClassEmbeddingLoader():
    ...
    def __init__(self, malware_samp_file, batch_size, shuffle_list=True, benign_ratio=0.01):
        super().__init__()
...
class Trainer():
    ...
    def compute_class_embeddings(self, data_file, vocab, token_max_len=512, data_parallel=True):
        ...
        for _, sample in enumerate(apk_iter_bar):
            ...
            with open(apk_file.replace('.txt', '.loc.pkl'), 'wb') as f:
                apk_label = max(class_label_list)
                pickle.dump([class_vector_list, class_label_list, class_name_list, apk_label], f)

    def train(self, data_file, vocab, batch_size, recompute_class_embeddings, token_max_len=512):
        """ Train Loop """
        ...
        for e in range(self.cfg.n_epochs):
            ...
            data_loader = ClassEmbeddingLoader(data_file, batch_size)
    ...
    def evaluate(self, eval_data_file, model_weights, vocab, recompute_class_embeddings=False, training_mode=True, token_max_len=512, k=10):
        if recompute_class_embeddings:
        ...
        data_loader = ClassEmbeddingLoader(eval_data_file, self.cfg.MCD_batch_size, shuffle_list=False)
        apk_iter_bar = tqdm(data_loader, desc='Iter (loss=X.XXX)')
...
def main(Bert_train_cfg='config/DexBERT/retrain.json',
         ...
         MalClassModel_file='../save_dir/malicious_code_localization/model_steps_347955.pt',
    ...
    if training_mode:
        trainer.train(data_file, vocab, MIL_cfg.MCD_batch_size, recompute_class_embeddings, max_len)
    if testing_mode:
        trainer.evaluate(data_file.replace('train', 'test'), MalClassModel_file, vocab, recompute_class_embeddings, training_mode, max_len)
    if time_cost:
